    ...
    <div class="coupons-detail">
        <ul class="coupons-list">
            <li>
                <div class="content">
                    <div class="left">
                        <p>{{detail.name}}</p>
                        <span>有效期：{{detail.begin_time.substr(0,16)}}-{{detail.end_time.substr(0,16)}}</span>
                    </div>
                    <i></i>
                    <div class="right">
        ...
        <Form class="common-form" :label-width="100" label-position="left" ref="form">
            ...
            <FormItem label="使用日期" v-show="use == 2">
                <Input v-model="detail.spending" :readonly="true"></Input>
            </FormItem>
            <FormItem label="使用门店" v-show="use == 2">
                <Input v-model="detail.tenantName" :readonly="true"></Input>
            </FormItem>
            <FormItem label="门店地址" v-show="use == 2">
                <Input v-model="detail.tenantAdd" :readonly="true"></Input>
            </FormItem>
            <FormItem label="服务网点">
                <router-link tag="span" :to="'/coupons-coms?id='+detail.companyIds"
                             class="ivu-input half select"></router-link>
            </FormItem>
        </Form>
        <Form class="common-form" :label-width="100" label-position="left" ref="form">
...
<script>
    ...
    export default {
        ...
        methods: {
            getList(code) {
                this.axiosHxx.post('/operate/coupon/returncouponmsg', {code: code, BUSINESS_TYPE: 1}).then(res => {
                    if (res.data.success) {
                        this.detail = res.data.data;
                        this.use = this.detail.isuse;
                    }
                });
            }
        },
